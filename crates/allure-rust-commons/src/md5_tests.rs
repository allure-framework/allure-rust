...
fn md5_rfc_1321_test_vectors() {
    allure_test(
        module_path!(),
        "md5_rfc_1321_test_vectors",
        "Verifies the MD5 implementation matches the RFC 1321 reference vectors.",
        || {
...
fn md5_common_known_strings() {
    allure_test(
        module_path!(),
        "md5_common_known_strings",
        "Verifies MD5 output for common strings used by Allure history and test identifiers.",
        || {
            assert_md5(
...
fn md5_boundary_lengths_around_single_block_padding() {
    allure_test(
        module_path!(),
        "md5_boundary_lengths_around_single_block_padding",
        "Verifies MD5 padding behavior around the single-block boundary.",
        || {
...
fn md5_binary_and_unicode_inputs() {
    allure_test(
        module_path!(),
        "md5_binary_and_unicode_inputs",
        "Verifies MD5 handles binary bytes and UTF-8 input consistently.",
        || {
...
fn md5_large_inputs() {
    allure_test(
        module_path!(),
        "md5_large_inputs",
        "Verifies MD5 remains correct for larger repeated inputs.",
        || {
...
fn md5_is_deterministic_across_repeated_calls() {
    allure_test(
        module_path!(),
        "md5_is_deterministic_across_repeated_calls",
        "Verifies repeated MD5 calculations for the same input produce the same digest.",
        || {
