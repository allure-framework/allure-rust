...
fn serializes_minimal_payload_and_omits_missing_optional_fields() {
    allure_test(
        ...
        "Verifies minimal HTTP exchanges omit absent optional fields.",
        || {
...
fn serializes_typical_payload() {
    allure_test(
        module_path!(),
        "serializes_typical_payload",
        "Verifies a representative HTTP exchange preserves request, response, headers, body, and timing data.",
        || {
            let mut exchange =
...
fn serializes_body_encoding_values() {
    allure_test(
        module_path!(),
        "serializes_body_encoding_values",
        "Verifies HTTP exchange body encoding variants serialize to their wire-format values.",
        || {
...
fn serializes_stream_type_field() {
    allure_test(
        module_path!(),
        "serializes_stream_type_field",
        "Verifies HTTP exchanges preserve the stream type field when it is provided.",
        || {
