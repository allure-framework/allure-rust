...
fn generates_allure_results_for_descriptions_sample() {
    ...
    let result = results
        .get("uses_doc_comment_as_description")
        .expect("missing uses_doc_comment_as_description result");
    assert_has_allure_result_fields(result);
    assert_eq!(json_string(result, "status"), Some("passed"));
    assert_eq!(
        json_string(result, "description"),
    ...
    let result = results
        .get("skips_doc_comment_description")
        .expect("missing skips_doc_comment_description result");
    assert_has_allure_result_fields(result);
    assert_eq!(json_string(result, "status"), Some("passed"));
    assert_eq!(json_string(result, "description"), None);
    assert_eq!(json_string(result, "descriptionHtml"), None);
...
fn generates_allure_results_for_links_sample() {
    allure::description("Verifies the links sample emits custom, issue, and TMS links in order.");
...
fn generates_allure_results_from_commons_runtime_functions() {
    ...
    assert!(result.contains("{\"uuid\":null,\"name\":\"log from commons\",\"status\":\"passed\""));
    assert!(
        result.contains("{\"uuid\":null,\"name\":\"logged from commons\",\"status\":\"passed\"")
    );
    assert!(
        result.contains("{\"uuid\":null,\"name\":\"attach from commons\",\"status\":\"passed\"")
    );
...
fn generates_allure_results_from_commons_test_runtime() {
    ...
    assert!(result
        .contains("{\"uuid\":null,\"name\":\"logged from manual runtime\",\"status\":\"passed\""));
    assert!(result
        .contains("{\"uuid\":null,\"name\":\"attach from manual runtime\",\"status\":\"passed\""));
...
fn selective_run_does_not_match_partial_selector_name() {
    allure::description("Verifies partial selector names do not accidentally select sample tests.");
