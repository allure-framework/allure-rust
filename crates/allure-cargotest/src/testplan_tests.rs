...
fn returns_none_when_env_is_unset() {
    allure_test(module_path!(), "returns_none_when_env_is_unset", || {
...
fn returns_none_when_file_does_not_exist() {
    allure_test(
        module_path!(),
        "returns_none_when_file_does_not_exist",
        || {
            let _guard = lock_testplan_env();
...
fn parses_plan_when_file_exists() {
    allure_test(module_path!(), "parses_plan_when_file_exists", || {
        let _guard = lock_testplan_env();
...
fn returns_none_for_malformed_json() {
    allure_test(module_path!(), "returns_none_for_malformed_json", || {
        let _guard = lock_testplan_env();
        let path = temp_file_path();
...
fn treats_empty_tests_as_unavailable() {
    allure_test(module_path!(), "treats_empty_tests_as_unavailable", || {
...
fn matches_by_exact_full_name_only() {
    allure_test(module_path!(), "matches_by_exact_full_name_only", || {
        let plan =
...
fn prefers_id_match_over_selector_within_entry() {
    allure_test(
        module_path!(),
        "prefers_id_match_over_selector_within_entry",
        || {
...
fn falls_back_to_metadata_tags_for_allure_id() {
    allure_test(
        module_path!(),
        "falls_back_to_metadata_tags_for_allure_id",
        || {
            let plan =
...
fn explicit_adapter_id_takes_precedence_over_tag_fallback() {
    allure_test(
        module_path!(),
        "explicit_adapter_id_takes_precedence_over_tag_fallback",
        || {
            let plan =
                parse_test_plan(r#"{"version":"1","tests":[{"id":"A-2"}]}"#).expect("valid plan");
